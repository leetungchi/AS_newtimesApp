...
public class PnlRptDiffAmtList
    //extends JPanel {
    extends AbsQryPanel{
...
protected  void jbInit() throws Exception {
    ...
    txtDateFrom.setText("");
    blueBkgWhiteCharLabel1.setText("Difference Amount List");
    ...
    slkBuyer.setBounds(new Rectangle(448, 49, 235, 25));
    slkMaker.setBounds(new Rectangle(83, 104, 235, 25));
    jLabel2.setBounds(new Rectangle(19, 99, 56, 30));
    jLabel2.setRequestFocusEnabled(true);
    jLabel2.setText("MAKER");
    slkBuyer1.setBounds(new Rectangle(447, 76, 235, 25));
    slkBuyer2.setBounds(new Rectangle(447, 104, 235, 25));
    ...
    this.add(slkBuyer2, null);
    slkBuyer.init("P_BUYER",new Integer(2078),slkBuyer.TYPE_BUYER,true);
    genDataBindObj();
    slkMaker.init("P_MAKER", null, slkMaker.TYPE_MAKER, true);
    slkBuyer1.init("P_BUYER", new Integer(12312), slkBuyer1.TYPE_BUYER, true);
    slkBuyer2.init("P_BUYER", new Integer(12227), slkBuyer2.TYPE_BUYER, true);
  }
  protected void genDataBindObj(){
    try{
      ...
      super.reportName="BUYER_DIFF_LIST.rpt";
    }catch(Exception exp){
       exp.printStackTrace();
    }
  }
  ...
  protected java.util.HashMap composeParameters(){
    ...
    if (slkBuyer.getSelectedValue() == null && slkBuyer1.getSelectedValue()  == null && slkBuyer2.getSelectedValue()  == null) {
         hm.put("P_BUYER","[0]");
    } else {
         String tmp_para="";
         if (slkBuyer.getSelectedValue() != null) {
             tmp_para=tmp_para+","+slkBuyer.getSelectedValue();
         }
         if (slkBuyer1.getSelectedValue() != null) {
            tmp_para=tmp_para+","+slkBuyer1.getSelectedValue();
        }
        if (slkBuyer2.getSelectedValue() != null) {
            tmp_para=tmp_para+","+slkBuyer2.getSelectedValue();
        }
        tmp_para="["+tmp_para.replaceFirst(",","")+"]";
        hm.put("P_BUYER",tmp_para);
    }
