package newtimes.reports.report_2.planqty;
...
public class PlanQty extends ScrollPaneGUI {
  Frame1 frame;
  private PlanQtyGUI gui = null;
  private String titleName = "Plan QTY Detail List (Not Yet Order)";
  private boolean isViewer = false;
  private java.util.HashMap hmFields = new java.util.HashMap();

  public PlanQty(Frame1 _frame) {
    frame = _frame;
    try {
      if(util.ApplicationProperites.getProperties("PlanQtyGUI")==null){
        gui = new PlanQtyGUI();
      }else{
        gui=(PlanQtyGUI)util.ApplicationProperites.getProperties("PlanQtyGUI");
      }
      jbInit();
    }
    catch(Exception ex) {
  ...
  public void do_Exit() {
    if (!isViewer) {
      util.ApplicationProperites.removeProperites("PlanQtyGUI");
      frame.showPanel(new newtimes.reports.report_2.ReportMenu2(frame));
    } else {
      frame.showPanel(new newtimes.reports.report_2.planqty.PlanQty(
          frame));
    }
  }
  ...
  public void do_Print() {
    try {
      if(checkField()){
        ...
        util.ApplicationProperites.setProperties("PlanQtyGUI", gui);
        this.butnPrint.setEnabled(false);
        isViewer = true;
        this.getFieldsData();
        setScrollPaneGUI(new exgui2.ReportGenerate("PLAN_QTY_NEW.rpt",
