...
public class UnShipped extends ScrollPaneGUI {
  Frame1 frame;
  private UnShippedGUI gui = null;
  private String titleName = "Unshipped Details by Quota";
  private boolean isViewer = false;
  private java.util.HashMap hmFields = new java.util.HashMap();

  public UnShipped(Frame1 _frame) {
    frame = _frame;
    try {
      if(util.ApplicationProperites.getProperties("UnShippedGUI")==null){
        gui = new UnShippedGUI();
      }else{
        gui=(UnShippedGUI)util.ApplicationProperites.getProperties("UnShippedGUI");
      }
      jbInit();
    }
    catch(Exception ex) {
  ...
  public void do_Exit() {
    if (!isViewer) {
      util.ApplicationProperites.removeProperites("UnShippedGUI");
      frame.showPanel(new newtimes.reports.report_2.ReportMenu2(frame));
    } else {
      frame.showPanel(new newtimes.reports.report_2.unshipped.UnShipped(
          frame));
    }
  }
  ...
  public void do_Print() {
    try {
      if(check_txtYear()){
        ...
        util.ApplicationProperites.setProperties("UnShippedGUI", gui);
        this.butnPrint.setEnabled(false);
        isViewer = true;
        this.getFieldsData();
        setScrollPaneGUI(new exgui2.ReportGenerate("UNSHIPPED_QTY.rpt",
  ...
  private String setReportAgument() {
    try {
      ...
      sb.append("&prompt6="+hmFields.get("QUOTA"));
      return sb.toString();
    } catch (Exception e) {
      e.printStackTrace();
      return null;
  ...
  java.util.HashMap getFieldsData() {
    try {
      ...
      hmFields.put("QUOTA", gui.dbJCBXQuota.getSelectedValue().equals("null")?"":gui.dbJCBXQuota.getSelectedValue());
