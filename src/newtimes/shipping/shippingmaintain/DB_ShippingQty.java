...
public class DB_ShippingQty extends PagedDataFactoryAdapter {
  ...
  public DB_ShippingQty() {
    ...
    System.out.println("compose the PRX_ClrLxyPrx hashMap");
     ...
     tempBlankRecord.set("SHIP_PRICE_PK", ((Record)ClrLixPrx.get(0)).get(0));
      ...
      Record sylte_rec = (database.datatype.Record)hm.get("recDetail");
      String extClmString=
          (String)sylte_rec.get("sty_size_column");
      ...
      BLANK_ORG_QTY_RECORD.set("record_create_user",
                          util.PublicVariable.USER_RECORD.get("usr_code"));
      unit_string =(String)((database.datatype.Record)hm.get("recDetail")).get("STY_UNIT");
      isInitOk=true;
    }catch(Exception e){
       e.printStackTrace();
  ...
  protected boolean isDzUnit(){
    try{
      Record rec=(database.datatype.Record)hm.get("recDetail");
      String unit=(String)rec.get("STY_UNIT");
  ...
  Record rec_org_shipping_style=(database.datatype.Record)hm.get("recDetail");
  byte sty_qty_type_num=(byte)rec_org_shipping_style.getInt("STY_QTY_TYPE");
