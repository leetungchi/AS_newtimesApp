...
public class QtyTableEditor extends PnlTableEditor {
  boolean init = false;
  ...
  public QtyTableEditor() {
    ...
    System.out.println("Actiave Direct QtyTableEditor()");
  }
  public boolean isIniOK(){
    return ((QtyDataFactoryAdapter)dbHandler).isDBHandlerInitOK();
  }
  ...
  protected InsertionOrderedHashMap getHeadColumnMap() {
    try {
      ...
      Vector vecSize = ((QtyDataFactoryAdapter)dbHandler).sizeType;
      if (!vecSize.isEmpty()) {
      ...
      byte sty_qty_type_num =
          Byte.parseByte(QtyDataFactoryAdapter.rec.get("STY_QTY_TYPE").toString());
  ...
  protected void refineEditors(MultiEditorJTable multiEditorJTableInSuper) {
    Vector priceVct = ((QtyDataFactoryAdapter)dbHandler).shipPrices;
    Vector licenseeVct = ((QtyDataFactoryAdapter)dbHandler).licensees;
    Vector colorVct = ((QtyDataFactoryAdapter)dbHandler).colors;
    try {
      byte sty_qty_type_num = Byte.parseByte(QtyDataFactoryAdapter.rec.get("STY_QTY_TYPE").toString());
  ...
  protected PagedDataFactory getPagedDataFactory() {
    dbHandler = new QtyDataFactoryAdapter();
    return dbHandler;
  }
  ...
  public void doAddNew()throws Exception{
    try {
      if (getJtable().getRowCount() > 0) {
        ...
        ((QtyDataFactoryAdapter)dbHandler).updateRecords(vct);
      }

      ((QtyDataFactoryAdapter)dbHandler).addRecord(null);
      this.goPage(1, 99999, false);
      ((QtyDataFactoryAdapter)dbHandler).udpateQtyTbl2Text(
  ...
  public void doUpdate() throws Exception{
    try {
      ...
      if (((QtyDataFactoryAdapter)dbHandler).hm.get("from").equals("AddDetailList")) {
        float totalValue = Float.parseFloat(totalValueObj.toString());
        float shipQty = Float.parseFloat(ShipDetailEdit.SHIP_QTY);
        float styleOrdQty = Float.parseFloat(ShipDetailEdit.STYLE_ORDER_QTY);
  ...
  public void doDelete()throws Exception{
    try {
      ...
      HashMap recMap = ((QtyDataFactoryAdapter)dbHandler).getMappedRec2TblCell();
      ...
      ((QtyDataFactoryAdapter)dbHandler).deleteRecords(vct2Delete);
  ...
  boolean isDzUnit() {
   try{
     if (testUnit == null) {
       testUnit = QtyDataFactoryAdapter.rec.get("STY_UNIT")==null?"":QtyDataFactoryAdapter.rec.get("STY_UNIT").toString();
     }
 ...
 synchronized boolean modifiedYYvalue(String str, int row, int column ) {
   ...
   try{
     ...
     if (!isOnlyOneEditingCell) {
       ...
       if (isDzUnit()) {
         orgVtotal = QtyDataFactoryAdapter.minusDzValue(orgVtotal, orgFloat);
         orgHtotal = QtyDataFactoryAdapter.minusDzValue(orgHtotal, orgFloat);
         totalFloat = QtyDataFactoryAdapter.minusDzValue(totalFloat, orgFloat);
         orgVtotal = QtyDataFactoryAdapter.addDzValue(orgVtotal, newQty);
         orgHtotal = QtyDataFactoryAdapter.addDzValue(orgHtotal, newQty);
         totalFloat = QtyDataFactoryAdapter.addDzValue(totalFloat, newQty);
       } else {
         orgVtotal -= orgFloat;
         orgHtotal -= orgFloat;
         totalFloat -= orgFloat;
        ...
        if (isDzUnit()) {
          ...
          for (int i = 0; i < rowcount; i++) {
            ...
            if (orgObjStr != null) {
              ...
              totalFloat = QtyDataFactoryAdapter.addDzValue(totalFloat, qtyValue);
            }
          }
        } else {
          int rowcount = getJtable().getRowCount() - 1;
  ...
  class myfldVfyObj implements exgui.ultratable.CellTxtEditorFormat,exgui.Object2String{
    ...
    public exgui.ultratable.KeyProcess getKeyListener(){
      return ((exgui.ultratable.CellTxtEditorFormat)
              exgui.verification.CellFormat.getDoubleFormat(
          ...
          if (isDzUnit()) {
            ...
            if (value != null) {
              ...
              String DecPartOfValue = util.MiscFunc.split(formatter.format(
                  xntFloatValue), ".")[1];
              if (!QtyDataFactoryAdapter.DecToDozeMapping.containsKey(
                  DecPartOfValue)) {
                warnningMsg =
  ...
  public Vector getData2Update(){
    HashMap mappedRecordInCell = ((QtyDataFactoryAdapter)dbHandler).getMappedRec2TblCell();
      ...
      for (int i = 0; i < rowcount; i++) {
        for (int j = 0; j < endColmn - 1; j++) {
          ...
          if (qtyObj != null && (!qtyObj.toString().trim().equals(""))) {
            ...
            QtyDataFactoryAdapter.Dz2Record(rec, qtyObj.toString());
          } else {
            QtyDataFactoryAdapter.Dz2Record(rec, 0);
          }
          ouptVct.add(rec);
        }
      }
    ...
    try {
      ...
      if (importHeadColumn.size() == 0) {
        ...
        for (int i = 0; i < (getJtable().getRowCount() - 1); i++) {
          ...
          if (isDzUnit()) {
            totalQtyValue = QtyDataFactoryAdapter.addDzValue(totalQtyValue, fltValue);
          } else {
        ...
        for (int j = 0; j < getJtable().getRowCount() - 1; j++) {
          for (int i = extendColumnBgnAt; i < extendColumnEndAt; i++) {
            ...
            if (isDzUnit()) {
              hzTotal[j] =
                  QtyDataFactoryAdapter.addDzValue(hzTotal[j], qtyValue);
              vtTotal[i - extendColumnBgnAt] =
                  QtyDataFactoryAdapter.addDzValue(vtTotal[i -
          ...
          if (isDzUnit()) {
            theEndTotal = QtyDataFactoryAdapter.addDzValue(theEndTotal, hzTotal[j]);
          } else {
