...
public class ToEditNthkProdHead extends ToEditAPreprodHEad {
  public ToEditNthkProdHead() {
  }
  public void toEditPreprodHead()throws Exception{
    ...
    try{
      ...
      if(newtimes.production.ProdMaintain_Properties.isAbleToEdit()){
        newtimes.production.process.prodmaintain.ToCheckB4StyleProc.setFunctionButtonStatus(
         ...
         (rec.getInt(0)>10)?true:false);//allow mis user to modify record
      }else{
        if(util.PublicVariable.OPERATTING_CENTER.equals("NTFE")){
          newtimes.production.process.prodmaintain.ToCheckB4StyleProc.setFunctionButtonStatus(false,false,false,false,false,false);
        }else{
          if(newtimes.production.ProdMaintain_Properties.IS_PO_SHIPPED.equalsIgnoreCase("true")){
            newtimes.production.process.prodmaintain.ToCheckB4StyleProc.setFunctionButtonStatus(
             false,true,true,false,false,
             (rec.getInt(0)>10)?true:false);
      ...
      //enable the priint button in edting mode,for NONE TPE user.
    ...
    /*
     ...
     javax.swing.JButton btn2style= processhandler.template.Properties.getSplitFrame().addExtraTriggerButton(
       "Styles",tgr);*/
    newtimes.production.process.prodmaintain.TgrGotoListNthkStyleFromProdHead  tgr=
        new newtimes.production.process.prodmaintain.TgrGotoListNthkStyleFromProdHead(pnl);
      javax.swing.JButton btn2style= processhandler.template.Properties.getSplitFrame().addExtraTriggerButton(
     "Styles",tgr);
