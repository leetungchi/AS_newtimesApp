...
public class DlgSequencialAddStylesNthk extends JDialog {
  JPanel panel1 = new JPanel();
  processhandler.template.PnlFieldsEditor  chainedPnlDftStyleValue1=null;
 ...
 newtimes.production.process.prodmaintain.DB_StyleListHandler db_StyleListHandler;
 boolean isToModifyStyles=false;
 public DlgSequencialAddStylesNthk (Frame frame, String title, boolean modal,
                              newtimes.production.process.prodmaintain.DB_StyleListHandler _db_StyleListHandler,
                              boolean isToModify) {

  super(frame, title, modal);
  if(isToModify)chainedPnlDftStyleValue1 = new ChanedPnlModifyStyleValuesNthk();
  try {
    setDefaultEditPanel();
    ...
    if(db_StyleListHandler!=null){
      ...
      if("NTHK".equals(util.PublicVariable.OPERATTING_CENTER)){
        blankStyle.set("sty_unit","DZS");
      }else{
        blankStyle.set("sty_unit","PCS");
      }
 ...
 protected void setDefaultEditPanel(){
    chainedPnlDftStyleValue1=new ChainedPnlDftStyleValueNthk();
 }
  public DlgSequencialAddStylesNthk(Frame frame, String title, boolean modal,
                                newtimes.production.process.prodmaintain.DB_StyleListHandler _db_StyleListHandler) {
    super(frame, title, modal);
    try {
      ...
      if(db_StyleListHandler!=null){
        chainedPnlDftStyleValue1.setPagedDataFactory(db_StyleListHandler);
        database.datatype.Record blankStyle=db_StyleListHandler.getBlankRecord();
        if("NTHK".equals(util.PublicVariable.OPERATTING_CENTER)){
          blankStyle.set("sty_unit","DZS");
        }else{
          blankStyle.set("sty_unit","PCS");
  ...
  public DlgSequencialAddStylesNthk() {
  ...
  public void setStyNo2Follow(String styNo2Follow){
    ((ChainedPnlDftStyleValueNthk)chainedPnlDftStyleValue1).setStyNo2Follow(styNo2Follow);
  }
  ...
  void btnOK_actionPerformed(ActionEvent e) {
    try{
      boolean result=((ChainedPnlDftStyleValueNthk)chainedPnlDftStyleValue1).btnOK_actionPerformed(null);
      if(!result) return;
