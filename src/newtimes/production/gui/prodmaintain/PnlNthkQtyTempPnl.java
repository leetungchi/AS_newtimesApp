...
public class PnlNthkQtyTempPnl extends PnlTableEditor
  ...
  public PnlNthkQtyTempPnl(DB_Trinity_PriceSizeQty _dbHandler) {
      super(null,0,null,null);
      super.dbHandler=_dbHandler;
      ...
      try{
        ...
        "Error In Generating Qty GUI\nPlease Contact System Manager",
        "Error In Generating Qty GUI");
      }
  }
  public PnlNthkQtyTempPnl() {
    super.jbtn1st.setVisible(false);
    super.jbtnLast.setVisible(false);
    super.jbtnNext.setVisible(false);
    super.jbtnPrev.setVisible(false);
  ...
  protected InsertionOrderedHashMap getHeadColumnMap() {
    ...
    try{
      ...
      String sizeColumns=
       ...
       String testImportHeadColumn[]=util.MiscFunc.split(sizeColumns,",,");
       Vector tmpVctSz=new Vector();
       for(int i=0; i<testImportHeadColumn.length;i++){
         if(testImportHeadColumn[i].trim().length()>0)tmpVctSz.add(testImportHeadColumn[i].trim());
       }
       importHeadColumn=(String[])tmpVctSz.toArray(new String[0]);

       if(tmpVctSz.size()==1 && tmpVctSz.get(0).toString().trim().equals("QTY")){
