...
public class PnlLIstShipPrxQty extends PnlTableEditor {
  ...
  public javax.swing.JButton btnDel=new javax.swing.JButton("DELETE");
  boolean ismodified=false;;
  public PnlLIstShipPrxQty() {
    ...
    btnAdd.addMouseListener(myMousAdpt);
    btnDel.addMouseListener(myMousAdpt);
    getLowerRightPanel().add(btnAdd);
    getLowerRightPanel().add(btnDel);
    getJtable().addMouseMotionListener(
      newtimes.production.ProdMaintain_Properties.getJTableToolTipGenerator(
      getJtable(),0));
  }
 ...
 //     }
      for(int i=1;i<getJtable().getColumnCount()-1;i++){
  ...
  class MyMousAdpt extends java.awt.event.MouseAdapter{
  ...
  public java.util.Vector getColorVct(){
    try{
      String modevalue = (String)newtimes.shipping_admin.ShippingAdmin_Const.
          EDITING_SHIPPING_STYLE_RECORD.get("_$SHIP_PACKING_AND_STYLE_MODE");
      if("P".equals(modevalue)){
        Vector vctColorCode=new Vector();
        ...
        sb.append("(select prod_color_pk from ship_price where SHIP_STYLE_PK=");
        sb.append(newtimes.shipping_admin.ShippingAdmin_Const.EDIT_SHIP_STYLE_SEQ_INT);
    ...
    return ((PrxQtyHandler)dbHandler).getColorVct();
    /*
    try{
      if (vctColorCode == null) {
        StringBuffer sb = new StringBuffer();
        sb.append("select prod_color_pk,prod_color_code,prod_color_name ");
        sb.append("from prod_color where prod_color_pk in ");
  ...
  public String[] getSizeColumn(){
    if(sizeColumn==null){
      try{
        boolean alreadyHaveColumn=true;
        String szClm=
            (String)newtimes.shipping_admin.ShippingAdmin_Const.EDITING_SHIPPING_STYLE_RECORD.get("ship_sty_size_column");

        if(szClm==null){
          alreadyHaveColumn=false;
         szClm = (String)newtimes.shipping_admin.ShippingAdmin_Const.
        ...
        if(!alreadyHaveColumn){
          newtimes.shipping_admin.ShippingAdmin_Const.EDITING_SHIPPING_STYLE_RECORD.set(
           "ship_sty_size_column",szClm);
        }
  ...
  protected Hashtable getObject2StringConvertor() {
    ...
    try{
      ...
      ht.put(COLUMN_CONTAINER  , new FormaterText());
  ...
  protected PagedDataFactory getPagedDataFactory() {
    ...
    if(dbHandler==null){
      PrxQtyHandler _dbHandler = new PrxQtyHandler(true);
 ...
 protected class FormaterText implements exgui.ultratable.CellTxtEditorFormat,exgui.Object2String{
   Object orgFvyObj=null;
   public FormaterText(){
     orgFvyObj=exgui.verification.CellFormat.getText_CaseSensitive(30);
   }
   public exgui.ultratable.KeyProcess getKeyListener(){
   ...
   public String Obj2String(Object obj2kvt){
     if(obj2kvt==null)return null;
     return obj2kvt.toString();

   }
    public Object cellStringToObject(String String2kvt){
      if(String2kvt==null)return null;
      if(String2kvt.trim().equals(""))return null;
  ...
  protected class FormaterPrx implements exgui.ultratable.CellTxtEditorFormat,exgui.Object2String{
   ...
   public String Obj2String(Object obj2kvt){
     ...
     if(Float.parseFloat(obj2kvt.toString())==0)return "0";
