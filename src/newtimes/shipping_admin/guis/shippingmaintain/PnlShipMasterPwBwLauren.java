...
public class PnlShipMasterPwBwLauren
  ...
  JLabel lblBill2 = new JLabel();
  JLabel lblShip2 = new JLabel();
  public PnlShipMasterPwBwLauren() {
    //super();
    try{
    jbInit();
    afterJbinit();
  ...
  protected void afterJbinit(){
    ...
    */
    lblBill2.setBackground(new Color(143, 143, 188));
    lblBill2.setFont(new java.awt.Font("Dialog", 1, 12));
    lblBill2.setForeground(new Color(10,36,106));
    lblBill2.setOpaque(true);
    lblBill2.setHorizontalAlignment(SwingConstants.LEFT);
    lblBill2.setText(" Bill to :");
    lblBill2.setBounds(new Rectangle(277, 4, 189, 20));
    lblShip2.setBounds(new Rectangle(550, 4, 173, 20));
    lblShip2.setText(" Ship to :");
    lblShip2.setHorizontalAlignment(SwingConstants.LEFT);
    lblShip2.setOpaque(true);
    lblShip2.setForeground(new Color(10,36,106));
    lblShip2.setFont(new java.awt.Font("Dialog", 1, 12));
    lblShip2.setBackground(new Color(143, 143, 188));
    ...
    jPanel1.add(lblBill2, null);
    jPanel1.add(lblShip2, null);
  }
  ...
  public void doAddNew(){
    ...
    try{
      //test if any SRN contains same FTY INV;ETD;Vessel&FLT ;& HAWB
      StringBuffer sbTest=new StringBuffer();
      String flight=util.MiscFunc.Replace( txtFLT.getText().trim(),"'","''");
      String vessel=util.MiscFunc.Replace( txtAreaVessel.getText().trim(),"'","''");
      String etd=txtShipETDdate.getText().trim();
      String hawb=util.MiscFunc.Replace( txtHAWB.getText().trim(),"'","''");
      String ftyInv=util.MiscFunc.Replace(txt_FTY_Inv.getText().trim(),"'","''");
      sbTest.append(" select ship_no from shipping where record_delete_flag='1' ");
      sbTest.append(" and length(ship_no)=13 ");
      sbTest.append("AND SUBSTR (shipping.ship_no, 4, 2) IN ('ND', 'PW', 'BW', 'FE') ");

      if(!"".equals(ftyInv)){
        sbTest.append(" and upper(SHIP_FTY_INV)='");
        sbTest.append(ftyInv.toUpperCase());
        sbTest.append("' ");
      }
      if(!"".equals(hawb)){
        sbTest.append(" and upper(SHIP_HAWB)='");
        sbTest.append(hawb.toUpperCase());
        sbTest.append("' ");
      }


      if(!"".equals(flight)){
        sbTest.append(" and upper(SHIP_FLT_NO)='");
        sbTest.append(flight.toUpperCase());
        sbTest.append("' ");
      }

      if(!"".equals(vessel)){
        sbTest.append(" and upper(SHIP_VESSEL)='");
        sbTest.append(vessel.toUpperCase());
        sbTest.append("' ");
      }

      if(!"".equals(etd)){
        sbTest.append(" and SHIP_ETD_DATE=to_date('");
        sbTest.append(etd);
        sbTest.append("', 'yyyy/mm/dd')");
      }

      java.util.Vector vctTestDup=exgui2.CONST.BASIC_MAIN_EJB.getDatas(
                   util.PublicVariable.USER_RECORD,
                   sbTest.toString(),1,9999999);
      if(vctTestDup.size()>0){
        StringBuffer sbMsg=new StringBuffer();
        for(int i=0;i<vctTestDup.size();i++){
          if(i>0)sbMsg.append(",");
          Record recTestDup=(Record)vctTestDup.get(i);
          sbMsg.append(recTestDup.get(0));
        }
        exgui.verification.VerifyLib.showAlert("Duplicated with SRN#:"+sbMsg.toString(),"Duplication");
        return;
      }
  ...
  public boolean gui2Record(){
    ...
    try{
      //check ETA > ETD date
      Object objShipEtaDate=recToMapping.get("ship_eta_date");
      if(null==objShipEtaDate){
        exgui.verification.VerifyLib.showAlert("Please Input ETA DATE",
                                               "Please Input ETA DATE");
        return false;
      }
      Object objShipEtdDate=recToMapping.get("ship_etd_date");
      if(!(txtShipETAdate.getText().equals(txtShipETDdate.getText())||
           ((java.util.Date)(objShipEtaDate)).after((java.util.Date)objShipEtdDate))
           ){
        exgui.verification.VerifyLib.showAlert("ETA Date Should Be Equal/Latter Than ETD Date",
                                               "ETA Date Should Be Equal/Latter Than ETD Date");
        return false;
      }
     ...
     exgui.DataBindEditableJCombobox DbJcbxcbx_IC_Div =
      ...
      vctShipType.add(recType3);
       ...
       lblIcInvType.setText(lblIcInvType.getText()+
        ...
        String asnNo=(String)recToMapping.get("SHIP_DIM_CUFT");
        if(null!=asnNo && asnNo.startsWith("ASN:")){
          asnNo=util.MiscFunc.Replace(asnNo.substring(4,asnNo.length()),"'","''");
          java.util.Vector vctAsn=exgui2.CONST.BASIC_MAIN_EJB.getDatas(util.PublicVariable.USER_RECORD,
              "select bill_to,ship_to from asn_upload where asn_no='"+asnNo+"' and rownum=1",1,1);
          if(vctAsn.size()>0){
            Record recAsn = (Record)vctAsn.get(0);
            Object bill2=recAsn.get(0);
            Object ship2=recAsn.get(1);
            lblBill2.setText((bill2==null)?"":bill2.toString());
            lblShip2.setText((ship2==null)?"":ship2.toString());
          }
        }
    }catch(Exception exp){
      exp.printStackTrace();
