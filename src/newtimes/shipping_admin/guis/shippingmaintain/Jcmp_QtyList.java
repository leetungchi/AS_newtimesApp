...
import newtimes.shipping_admin.process.shiippingmaintain.*;
...
implements javax.swing.event.TableModelListener{
  ...
  protected java.util.HashMap colorLicensesPriceMap;
  javax.swing.text.JTextComponent _txt2ShowAsccTable;
  javax.swing.JButton btnGetBalace=new javax.swing.JButton("Get Balance");
  public Jcmp_QtyList(javax.swing.text.JTextComponent txtObj) {
    ...
    priceVct=((DB_ShippingStyleQty)dbHandler).Prices;
    licenseeVct=((DB_ShippingStyleQty)dbHandler).Licensees;
    colorVct=((DB_ShippingStyleQty)dbHandler).ProdColors;
    formatter=new java.text.DecimalFormat("#######.##");
    Dzformatter=new java.text.DecimalFormat("######0.00");
    _txt2ShowAsccTable=txtObj;
    if(!util.PublicVariable.OPERATTING_CENTER.equals("NTFE")){
      super.getLowerRightPanel().add(btnGetBalace, null);
    }

    btnGetBalace.addActionListener(new java.awt.event.ActionListener() {
  ...
  public void initQtyByBalance(){
    try{
          //util.MiscFunc.showProcessingMessage();
          java.util.HashMap balanceQtyMap=((DB_ShippingStyleQty)dbHandler).getBalanceQtyMap();
          java.util.HashMap initKeyMap=((DB_ShippingStyleQty)dbHandler).PRICE_PK_MAP_COLOR_LICENSEE;
          Vector vctQty=getDisplayingRecords();
          JTable tbl=getJtable();
          boolean isOnlyQty=tbl.getColumnName(tbl.getColumnCount()-1).equalsIgnoreCase("QTY");
          int endColumnCount=(isOnlyQty)?(priceColumnAt+1):(tbl.getColumnCount()-2);
          for(int i=0;i<tbl.getRowCount()-1;i++){
            //System.out.println("contet of displaying qty record: "+((Record)vctQty.get(i)).toString());
            String pkPrice=((Record)vctQty.get(i)).get("_SHOW_LXY_COLOR").toString();
            String color_licensee_pk=(String)initKeyMap.get(pkPrice);
            for(int j=priceColumnAt+1;j<(endColumnCount+1);j++){
              String key4Balance=
                  color_licensee_pk+tbl.getColumnName(j).trim()+((DB_ShippingStyleQty)dbHandler).TAIL_STRING_OF_COLOR_LICENSEE_KEY;
              //System.out.println("get key for balance:"+key4Balance);
              Float floatobj=(Float)balanceQtyMap.get(key4Balance);
              //System.out.println("get value for balance:"+floatobj);
              tbl.setValueAt(floatobj.toString(),i,j);
            }
          }
          sumEveryColumn();
    }catch(Exception exp){
      ...
      exgui.verification.VerifyLib.showAlert("Error While Generating Balance Qty\nPlease Contact System Manager",
                                             "Error In Init-Qty By Balanace");
    }finally{
         //util.MiscFunc.hideProcessingMessage();
    }
  }
  public boolean isIniOK(){
    return ((DB_ShippingStyleQty)dbHandler).isDBHandlerInitOK();
  }
    ...
    try{
      ...
      if(importHeadColumn.length==1 && importHeadColumn[0].trim().length()==0){
       ...
       byte sty_qty_type_num=(byte)
           newtimes.shipping_admin.ShippingAdmin_Const.EDITING_SHIPPING_STYLE_RECORD.getInt("_$ORG_STY_QTY_TYPE");
      //----------for future to compare----------
       ...
       *///don't care licensee.
       ...
       if(firstColum2Show.toString().trim().length()!=0){
         iso.put(firstColum2Show.toString(),
                 DB_ShippingStyleQty._SHOW_LXY_COLOR);
       }
  ...
  protected void refineEditors(MultiEditorJTable multiEditorJTableInSuper) {
    initializing=true;
    try{
      colorLicensesPriceMap=((DB_ShippingStyleQty)dbHandler).getPriSeqMapPrx();
      //if(licenseeColorsPrice==null)
      licenseeColorsPrice=((DB_ShippingStyleQty)dbHandler).getClrLxyPrxs();

      byte sty_qty_type_num =
          (byte)newtimes.shipping_admin.ShippingAdmin_Const.EDITING_SHIPPING_STYLE_RECORD.getInt("_$ORG_STY_QTY_TYPE");
  ...
  public void tableChanged(javax.swing.event.TableModelEvent te){
    ...
    HashMap tempHm=colorLicensesPriceMap;//
  ...
  protected PagedDataFactory getPagedDataFactory() {
    dbHandler=new DB_ShippingStyleQty();
    licenseeColorsPrice=((DB_ShippingStyleQty)dbHandler).getClrLxyPrxs();
    return dbHandler;
  }
 public Vector getData2Update(){
   HashMap mappedRecordInCell=
       ((DB_ShippingStyleQty)dbHandler).getMappedRec2TblCell();
     ...
     for (int i = 0; i < rowcount; i++) {
      ...
      Object qtyObj = _jtbl.getValueAt(i, j + priceColumnAt + 1).toString();
         if (qtyObj != null && (!qtyObj.toString().trim().equals(""))) {
           ...
           DB_ShippingStyleQty.Dz2Record(rec, qtyObj.toString());
         }
         else {
           DB_ShippingStyleQty.Dz2Record(rec, 0);
         }
 ...
 public void doDelete()throws Exception{
  ...
  if (getJtable().getRowCount() > 0) {
    ...
    ((DB_ShippingStyleQty)dbHandler).updateRecords(vct);
  }
  ...
  HashMap recMap = ((DB_ShippingStyleQty) dbHandler).getMappedRec2TblCell();
  ...
  ((DB_ShippingStyleQty) dbHandler).deleteRecords(vct2Delete);
  //exgui.verification.VerifyLib.showPlanMsg("Delete OK", "Delete OK");
  this.goPage(1, 99999, false);
  String fianlTableText=newtimes.production.ProdMaintain_Properties.table2Text(getJtable());
  ((DB_ShippingStyleQty) dbHandler).udpateQtyTbl2Text(fianlTableText);
   _txt2ShowAsccTable.setText(fianlTableText);
 ...
 protected boolean isOverShip()throws Exception{
  ...
  sumEveryColumn();
  Object totalOfCurrentQty=getJtable().getValueAt(
 ...
 public void doAddNew()throws Exception{
   try{
     ...
     ( (DB_ShippingStyleQty) dbHandler).addRecord(null);
     this.goPage(1, 99999, false);
     String finallTableText=newtimes.production.ProdMaintain_Properties.table2Text(getJtable());
     ((DB_ShippingStyleQty) dbHandler).udpateQtyTbl2Text(
      finallTableText
      );
      /*
      exgui.verification.VerifyLib.showPlanMsg("Add New Record OK",
                                              "Add New OK");
      */
      _txt2ShowAsccTable.setText(finallTableText);
   }finally{
     util.MiscFunc.hideProcessingMessage();
     newtimes.production.ProdMaintain_Properties.setNormalCursor();
   }
    ...
    newtimes.production.ProdMaintain_Properties.setWaitCursor();
     ...
     this.goPage(1, 99999, false);
      String defaultStyleDownDescription=null;
       ...
       _txt2ShowAsccTable.setText(defaultStyleDownDescription);
       ...
       if(importHeadColumn.length==0){
         ...
         for(int i=0;i<(getJtable().getRowCount()-1);i++){
          Object qtyObj=getJtable().getValueAt(i,getJtable().getColumnCount()-1);
           ...
           if(isDzUnit()){
             totalQtyValue=DB_ShippingStyleQty.addDzValue(totalQtyValue,fltValue);
           }else{
         ...
         for(int j=0;j<getJtable().getRowCount()-1;j++){
           for(int i=extendColumnBgnAt;i<extendColumnEndAt;i++){
                ...
                if(isDzUnit()){
                  hzTotal[j]=
                      DB_ShippingStyleQty.addDzValue(hzTotal[j],qtyValue);
                  vtTotal[i-extendColumnBgnAt]=
                      DB_ShippingStyleQty.addDzValue(vtTotal[i-extendColumnBgnAt],qtyValue);
                }else{
           ...
           if(isDzUnit()){
             theEndTotal=DB_ShippingStyleQty.addDzValue(theEndTotal,hzTotal[j]);
           }else{
 ...
 protected boolean isDzUnit(){
   try{
     if(testUnit==null){
       testUnit=
        (String)newtimes.shipping_admin.ShippingAdmin_Const.EDITING_SHIPPING_STYLE_RECORD.get("_$ORG_STYLE_UNIT");
     }
   ...
   try{
    ...
    float orgFloat=MyFloatPars(objOrgQty);
     if(!isOnlyOneEditingCell){
       ...
       if(isDzUnit()){
         orgVtotal=DB_ShippingStyleQty.minusDzValue(orgVtotal,orgFloat);
         orgHtotal=DB_ShippingStyleQty.minusDzValue(orgHtotal,orgFloat);
         totalFloat=DB_ShippingStyleQty.minusDzValue(totalFloat,orgFloat);
         orgVtotal=DB_ShippingStyleQty.addDzValue(orgVtotal,newQty);
         orgHtotal=DB_ShippingStyleQty.addDzValue(orgHtotal,newQty);
         totalFloat=DB_ShippingStyleQty.addDzValue(totalFloat,newQty);
       }else{
         orgVtotal-=orgFloat;
         orgHtotal-=orgFloat;
         totalFloat-=orgFloat;
        ...
        if(isDzUnit()){
          ...
          for(int i=0;i<rowcount;i++){
            ...
            if(orgObjStr!=null){
              ...
              totalFloat=DB_ShippingStyleQty.addDzValue(totalFloat,qtyValue);
            }
          }
        }else{
          int rowcount=getJtable().getRowCount()-1;
               ...
               //Double d = new Double(((Number)formatter.parse((String)inStr)).doubleValue());
                  if(isDzUnit()){
                    ...
                    if(value!=null){
                      ...
                      if(!DB_ShippingStyleQty.DecToDozeMapping.containsKey(DecPartOfValue)){
