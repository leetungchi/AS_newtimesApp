...
public class DivisionList extends JPanel {
  Frame1 frame;
  private JButton butnAdd = new JButton();
  private JButton butnExit = new JButton();
  private JLabel textAreaStatus = new JLabel();
  private JLabel jLabel1 = new JLabel();
  private DivisionAbsTable divAbsTable = new DivisionAbsTable();
  private JTable jTable = divAbsTable.getJtable();

  public DivisionList(Frame1 _frame) {
    frame = _frame;
    try {
      jbInit();
    }
  ...
  void jbInit() throws Exception {
    if (DivisionEdit.isExit) {
      ...
      divAbsTable.goPage(_rowPosition,_rowCounts,true);
      /*
      int skip = Math.abs(_rowPosition/_rowCounts);
      for (int i = 0; i < skip; i++) {
        divAbsTable.nextPage();
      }*/
    }
    ...
    jLabel1.setText("      ============  Division List  ============      ");
    jLabel1.setBounds(new Rectangle(117, 26, 566, 24));
    divAbsTable.setBounds(new Rectangle(16, 82, 760, 406));
    this.add(butnAdd, null);
    this.add(textAreaStatus, null);
    this.add(jLabel1, null);
    this.add(butnExit, null);
    this.add(divAbsTable, null);
  }
  void butnAdd_actionPerformed(ActionEvent e) {
    setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
    frame.showPanel(new newtimes.basic.division.DivisionAdd(frame));
    setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
  }
  void butnExit_actionPerformed(ActionEvent e) {
    setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
    DivisionEdit.isExit = false;
  ...
  void jTable_mouseReleased(MouseEvent e) {
    ...
    try {
      divAbsTable.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
      util.ApplicationProperites.setProperties("_rowCounts", new Integer(divAbsTable._rowCounts));
      util.ApplicationProperites.setProperties("_rowPosition", new Integer(divAbsTable._rowPosition));
      int getRow = jTable.getSelectedRow();
      java.util.Vector vDetail = divAbsTable.getDisplayingRecords();
      ...
      frame.showPanel(new newtimes.basic.division.DivisionEdit(frame));
    } catch (Exception eRow) {
      eRow.printStackTrace();
      util.ExceptionLog.exp2File(eRow, "error at get record." + hm.toString());
    } finally {
      divAbsTable.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
    }
  }
}
