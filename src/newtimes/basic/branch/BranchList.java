...
public class BranchList extends JPanel {
  Frame1 frame;
  private JButton butnExit = new JButton();
  private JLabel textAreaStatus = new JLabel();
  private JLabel jLabel1 = new JLabel();
  private JButton jButton1 = new JButton();
  private BranchAbsTable branchAbsTable = new BranchAbsTable();
  private JTable jTable = branchAbsTable.getJtable();

  public BranchList(Frame1 _frame) {
    frame = _frame;
    try {
      jbInit();
    }
  ...
  void jbInit() throws Exception {
    if (BranchEdit.isExit) {
      ...
      branchAbsTable.goPage(_rowPosition,_rowCounts,true);
      /*
      int skip = Math.abs(_rowPosition/_rowCounts);
      for (int i = 0; i < skip; i++) {
        branchAbsTable.nextPage();
      }*/
    }
    ...
    jLabel1.setText("      ============  Branch List  ============      ");
    jLabel1.setBounds(new Rectangle(117, 26, 566, 24));
    jButton1.setBackground(Color.lightGray);
    jButton1.setBounds(new Rectangle(285, 503, 92, 25));
    ...
    branchAbsTable.setBounds(new Rectangle(16, 82, 760, 406));
    this.add(textAreaStatus, null);
    this.add(jLabel1, null);
    this.add(butnExit, null);
    this.add(jButton1, null);
    this.add(branchAbsTable, null);
  }

  void butnExit_actionPerformed(ActionEvent e) {
    setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
    BranchEdit.isExit = false;
  ...
  void jButton1_actionPerformed(ActionEvent e) {
    setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
    frame.showPanel(new newtimes.basic.branch.BranchAdd(frame));
    setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
  }
  ...
  void jTable_mouseReleased(MouseEvent e) {
    ...
    try {
      branchAbsTable.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
      util.ApplicationProperites.setProperties("_rowCounts", new Integer(branchAbsTable._rowCounts));
      util.ApplicationProperites.setProperties("_rowPosition", new Integer(branchAbsTable._rowPosition));
      int getRow = jTable.getSelectedRow();
      java.util.Vector vDetail = branchAbsTable.getDisplayingRecords();
      database.datatype.Record recDetail = (database.datatype.Record) vDetail.
          get(getRow);
      hm.put("recDetail", recDetail);
      util.ApplicationProperites.setProperties("recDetail", hm);
      frame.showPanel(new newtimes.basic.branch.BranchEdit(frame));
    } catch (Exception eRow) {
      eRow.printStackTrace();
      util.ExceptionLog.exp2File(eRow, "error at get record." + hm.toString());
    } finally {
      branchAbsTable.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
    }
  }
}
