...
public class PriceTermList extends JPanel {
  Frame1 frame;
  private JLabel textAreaStatus = new JLabel();
  private JButton butnExit = new JButton();
  private JLabel jLabel1 = new JLabel();
  private JButton butnAdd = new JButton();
  private PriceTermAbsTable ptAbsTable = new PriceTermAbsTable();
  private JTable jTable = ptAbsTable.getJtable();

  public PriceTermList(Frame1 _frame) {
    frame = _frame;
    try {
      jbInit();
    }
  ...
  void jbInit() throws Exception {
    if (PriceTermEdit.isExit) {
      ...
      ptAbsTable.goPage(_rowPosition,_rowCounts,true);
      /*
    ...
    butnAdd.setBounds(new Rectangle(299, 505, 91, 25));
    ...
    jLabel1.setText("      ============  Price Term List  ============      ");
    jLabel1.setBounds(new Rectangle(117, 26, 566, 24));
    butnExit.setBackground(Color.lightGray);
    butnExit.setBounds(new Rectangle(409, 505, 80, 25));
    ...
    ptAbsTable.setBounds(new Rectangle(16, 82, 760, 406));
    this.setLayout(null);
    ...
    this.add(ptAbsTable, null);
  }

  void butnExit_actionPerformed(ActionEvent e) {
    setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
    PriceTermEdit.isExit = false;
  ...
  void butnAdd_actionPerformed(ActionEvent e) {
    setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
    frame.showPanel(new newtimes.basic.priceterm.PriceTermAdd(frame));
    setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
  }
  ...
  void jTable_mouseReleased(MouseEvent e) {
    ...
    try {
      ptAbsTable.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
      util.ApplicationProperites.setProperties("_rowCounts", new Integer(ptAbsTable._rowCounts));
      util.ApplicationProperites.setProperties("_rowPosition", new Integer(ptAbsTable._rowPosition));
      int getRow = jTable.getSelectedRow();
      java.util.Vector vDetail = ptAbsTable.getDisplayingRecords();
      ...
      frame.showPanel(new newtimes.basic.priceterm.PriceTermEdit(frame));
    } catch (Exception eRow) {
      eRow.printStackTrace();
      util.ExceptionLog.exp2File(eRow, "error at get record." + hm.toString());
    } finally {
      ptAbsTable.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
    }
  }
}
