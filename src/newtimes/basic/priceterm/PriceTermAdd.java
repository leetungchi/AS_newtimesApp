...
public class PriceTermAdd extends JPanel {
  Frame1 frame;
  ...
  public PriceTermAdd(Frame1 _frame) {
    frame = _frame;
    try {
      jbInit();
    }
  ...
  void jbInit() throws Exception {
    ...
    jLabel1.setText("      ============  Price Term Add  ============      ");
  ...
  void butnAdd_actionPerformed(ActionEvent e) {
    ...
    if (this.checkFieldsData()) {
      ...
      try {
        hm.put("PRC_TRM", txtPriceTerm.getText().trim().toUpperCase());
        hm.put("create_user", util.PublicVariable.USER_RECORD.get("USR_CODE"));
        int isOk = exgui2.CONST.BASIC_MAIN_EJB.insertPriceTerm(hm);
