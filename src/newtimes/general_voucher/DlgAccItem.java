...
public class DlgAccItem extends JDialog {
  ...
  public DlgAccItem(Frame frame, String title, boolean modal) {
  ...
  class DummyDB4PayeeItem extends exgui.ultratable.PagedDataFactoryAdapter{
    Vector vctData;
    newtimesejb.GnVoucher.GnVoucherFacade gnVoucherFacade;
    ...
    DummyDB4PayeeItem(){
       try{
         ...
         gnVoucherFacade=ejbHome.create();
         blankRecord= exgui2.CONST.BASIC_MAIN_EJB.getBlankRecord("GV_ACC_ITEM_tbl");
         ...
         vctData=exgui2.CONST.BASIC_MAIN_EJB.getDatas(util.PublicVariable.USER_RECORD,
                                                      "select * from GV_ACC_ITEM where record_delete_flag='1' order by item_code",
                                                      1,99999);
       }catch(Exception exp){
         exp.printStackTrace();
    ...
    public void finalUpdate(Vector vct)throws Exception{
      gnVoucherFacade.handleGnVoucherAccItem(vct);
    }
  }
  private void jbInit() throws Exception {
    dbPayeeItemHandler=new DummyDB4PayeeItem();
    util.InsertionOrderedHashMap iso=new util.InsertionOrderedHashMap();
    iso.put("代碼","ITEM_CODE");
    iso.put("D/C","D_C");
    iso.put("       名稱      ","ITEM_NAME");

    Hashtable ht=new Hashtable();
    Object char30Vfy=exgui.verification.CellFormat.getOrdinaryField(25);
    Object char4Vfy=exgui.verification.CellFormat.getOrdinaryField(4);
    Object char1Vfy=exgui.verification.CellFormat.getOrdinaryField(1);
    ...
    pnl2List.setBounds(new Rectangle(28, 28, 350,272));
    btnOK.setText("OK");
  ...
  void btnOK_actionPerformed(ActionEvent e) {
     ...
     for( int i=0;i<dbPayeeItemHandler.vctData.size();i++){
       ...
       if(rec.getModifiedFields().size()>0){
         if(rec.getInt(6)<0)rec.set(4,util.PublicVariable.USER_RECORD.get(0));//create user
         rec.set(4,util.PublicVariable.USER_RECORD.get(0));//modify user
         vct2update.add(rec);
       }
     }
